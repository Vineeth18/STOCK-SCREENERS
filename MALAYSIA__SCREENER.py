...
def get_stock_price(ticker):
  ...
  '''Abstract Syntax Tree (AST) is a structured representation of the syntax of a program.
  ast.literal_eval() function is used to safely evaluate Python literals from a string 
  without running the risk of executing potentially harmful code. It's often employed in situations 
  where you want to evaluate a literal value provided as a string without introducing security vulnerabilities.'''
  ...
  '''the Unix timestamps are converted to UTC time 
  a way of representing a specific point in time as the number of seconds that have passed 
  since a fixed reference point known as the "Unix epoch." 
